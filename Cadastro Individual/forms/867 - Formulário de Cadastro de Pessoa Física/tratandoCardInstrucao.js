$(function () {
  tratandoCardInstrucao();
});

function tratandoCardInstrucao() {
  var VISIBILIDADE = buscarModoForm();
  var ATIVIDADE_ATUAL = buscarAtividadeAtual();

  if (ATIVIDADE_ATUAL == 12 || ATIVIDADE_ATUAL == 0) {
    aparecerCardInstrucaoUsuario();
    aparecerCardInstrucaoAnexo();
  } else if (ATIVIDADE_ATUAL == 5) {
    removerCardInstrucaoUsuario();
    removerCardInstrucaoAnexo();
...
function aparecerCardInstrucaoUsuario() {
...
function aparecerCardInstrucaoAnexo() {
...
// Remove instruções
function removerCardInstrucaoUsuario() {
  $("[id=card-instrucao-usuario]").remove();
}

function removerCardInstrucaoAnexo() {
